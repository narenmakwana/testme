...
#-----------------------------------------------------------------
# daytrader_cluster.py - DayTrader Cluster Install Script
#-----------------------------------------------------------------
#
...
DefaultClusterName = "TradeCluster"
DefaultClusterDescription = "Trade Cluster Config"
DefaultClusterMember = "TradeServer"
DefaultMemberWeight = "2"
DefaultPreferLocal = "true"


ClusterNodeNames = ["nodeName1", "nodeName2"]
ClusterMemberNames = ["serverName1", "serverName2"]
...
DefaultTradeAppName = "DayTrader2-EE5"
DefaultEarFile =      "daytrader20-ee5.ear"
...
if (SilentInstall == "false" and ( operation == "configure" or operation == "all") ):
	...
	cell = getCellId()
	TargetCellName = getName(cell)
	scope = cell

	print " "
	print "-------------------------------------------------"
	print " Collecting Cluster and Cluster Member Info"
	print " "
	print " Note: Before proceeding, all nodes intended for"
	print "  use in this cluster must be federated with the"
	print "  deployment manager using the addNode command!"
	print "  To ensure that this process goes smoothly, it"
	print "  is also important to verify that each node can"
	print "  ping the other cluster nodes based on the host"
	print "  names configured within the WebSphere profile"
	print "  creation tool."             
	print "-------------------------------------------------"

	goOn = getValidInput("Have all nodes been federated and network connectivity verified? (yes|no) [yes]: ","yes",DefaultOptions)  
	if (goOn == "no"):
		sys.exit()
	#endIf

	# Obtain cluster name and create cluster
	DefaultClusterName =getInput("Please enter the cluster name [" + DefaultClusterName + "]:", DefaultClusterName)
	...
	while addNodes == "yes":
		...
		if (AdminTask.getNodePlatformOS( "-nodeName " + TargetNodeName )  == "os390"):
			print "--------------------------------------------------"
			print " Note: Your node you have selected is installed on z/OS."
			print " Daytrader installations on z/OS platforms are"
			print " currently limited to single server environments."
			print " Exiting the install script!"
			print "--------------------------------------------------"
			sys.exit()
		else:
			ClusterNodeNames.append(getName(node))
			countNodes = len(ClusterNodeNames)

			member = getInput("Please enter the cluster member name [" + DefaultClusterMember + str(countNodes) + "]:", DefaultClusterMember + str(countNodes))
			ClusterMemberNames.append(member)
		#endElse

		# List cluster node/member pairs
		print ""
		print "Current Cluster Nodes and Members:"
		idx = 0
		for item in ClusterNodeNames :
			tmp = ClusterMemberNames[idx]
			idx = (idx + 1)
			print item + " - " + tmp
		#endFor

		addNodes = getValidInput ("Add more cluster members (yes|no) [yes]:","yes",DefaultOptions)
	#endwhile
	print "Cluster information obtained..."
	...
	DefaultMEDatabaseName = getInput("Please enter the ME database name (location) ["+DefaultMEDatabaseName+"]:", DefaultMEDatabaseName )
...
if (operation == "all" or operation == "configure"):
	...
	# Create the cluster
	cluster = createCluster (DefaultClusterName,DefaultPreferLocal,DefaultClusterDescription,scope)
	idx = 0
	for nodes in ClusterNodeNames:
		node = AdminConfig.getid("/Node:" + nodes + "/")
		member = ClusterMemberNames[idx]
		idx = idx + 1

		# Create the cluster  member
		clusterMember = createClusterMember(member,node,DefaultMemberWeight,cluster) 
	...
	#provider = createJDBCProvider(DefaultProviderType, DefaultXA, scope, DefaultPathName, DefaultNativePathName )
	datasource = createDatasource(DefaultClusterMEDatasourceName, "jdbc/"+DefaultClusterMEDatasourceName, DefaultStmtCacheSize, DefaultDatasourceAuthAliasName, provider)

	if (DefaultProviderType.find("DB2") >= 0 or DefaultProviderType == "Derby"):
		updateDB2orDerbyDatasource(datasource, DefaultMEDatabaseName, DefaultHostname, DefaultPort, DefaultDB2DriverType)
	elif (DefaultProviderType == "Oracle"):
		updateOracleDatasource(datasource, DefaultMEDatabaseName, DefaultHostname, DefaultPort)
	elif (DefaultProviderType == "Informix"):
		updateInformixDatasource(datasource, DefaultMEDatabaseName, DefaultIfxServerName, DefaultPort, DefaultHostname, DefaultIfxLockMode)
	elif (DefaultProviderType == "Embedded MS SQL Server"):
		print "Not yet supported"
		sys.exit()
	#endIf
	...
	sibus = createSIBus(DefaultClusterName,DefaultAdminAuthAliasName)
	target = [DefaultClusterName]
	dsParms = ["false","jdbc/"+DefaultClusterMEDatasourceName] 
	addSIBusMember(sibus, "false", target, dsParms)
	...
	for engine in meList.split("\n") :
		...
		if (DefaultProviderType == "Oracle") :
			# For oracle, schema and username are essentially the same thing. Before installation,
			# users for each of the ME datastores must be created with the same password as the "trade"
			# user. The naming format for these is IBMME0, IBMME1, ..., IBMME(N). These users should
			# be granted privileges that allow table and index creation within their respective schemas. 
			alias = schema + "AuthAlias"
			createJAASAuthData(alias,schema,DefaultPasswd)
			modifyMEDataStore(engine,alias,schema)            
		else :
...
if (operation == "cleanup"):
	...
	deleteSIBus(DefaultClusterName)
	...
	if (len(policyList) > 0):
		for item in policyList.split("\n"):
			...
			if ((item[0:14]) == '\"Policy for ME' ):
				deleteOneOfNPolicy(getName(item))
			#endIf
		#endFor
	#endIf
